...
public:
    ...
    /**
     * Connect to server via connect token.
     * This is the secure connection path. Typically the connect token is generated by the matchmaker.
     */

    void SnapshotSecureConnect(uint8_t* ConnectToken);

    /**
     * Connect to server via IP address.
     * This is the INSECURE connection path. This should only be used in dev.
     */

    void SnapshotInsecureConnect(const FString& Host, int32 Port);
