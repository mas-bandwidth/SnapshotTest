...
void FSnapshotSocketClient::SnapshotSecureConnect(uint8_t* connect_token)
{
    ...
    // If the connect token is invalid, it will fail here
...
#if 0
    // Get the server address we are connecting to (it is included in the connect token...)
    ...
    snapshot_address_to_string(server_address, snapshot_address_buffer);
    uint8_t address_type = server_address->type;
    uint16_t address_port = server_address->port;
    ...
    if (!bIsValid)
    {
        UE_LOG(LogSnapshot, Error, TEXT("Invalid server address '%s'"), *SnapshotAddressAsUnrealString);
        return;
    }
#endif // #if 0
...
void FSnapshotSocketClient::SnapshotInsecureConnect(const FString& Host, int32 Port)
{
    ...
    // todo
    snapshot_printf(SNAPSHOT_LOG_LEVEL_DEBUG, "server address is '%s'", server_address_buffer);
...
bool FSnapshotSocketClient::SendTo(const uint8* Data, int32 Count, int32& BytesSent, const FInternetAddr& Destination)
{
    ...
    if (!bConnected)
    {
        SnapshotInsecureConnect("127.0.0.1", 17777);
    }
